...
module.exports = function (deployer, network) {
    ...
    if (network != 'kovan') {
        ...
        deployer.deploy(BancorNetwork, CONF.registry_address).then(async() => {
            await deployer.deploy(BancorConverter, CONF.ring_address, CONF.registry_address, 0, EtherToken.address, CONF.weight10Percent, {gas: 7000000});
        }).then(async() => {
            await deployer.deploy(BancorExchange, BancorNetwork.address, BancorConverter.address, CONF.registry_address, {gas: 3000000});
        }).then(async () => {
            ...
            registry.setAddressProperty(ringId, CONF.ring_address);
